...
This is the main paper behind the JumpReLU style training of the Gemma Scope SAEs, which we know are simply a modification to the ReLU functio, with a learned $\theta$ parameter. Since JumpReLU applies 0 to activations below the learned parameter, to backpropagate, we need STE (Straight Through Estimator).
