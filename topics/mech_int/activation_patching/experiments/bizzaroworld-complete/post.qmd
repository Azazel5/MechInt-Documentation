---
title: "BizzaroWorld, complete replication and findings on Gemma2B and Gemma12B-IT"
format:
  html:
    toc: true
...
- **Phase 2 - Routing (distributed attention heads):** Signal 
  ...
  across Mode A, but individual head damage (max $\Delta$LD = -0.68) was 
  negligible compared to full entity damage ($\Delta$LD = -11.47).
...
![Gemma 2B attention head routing — per-head logit difference contributions across layers; signal is distributed with no dominant single head (max contribution ~23% of total swing)](./figures/Gemma2B-head-routing.png){fig-alt="Heatmap of per-head path patching scores for Gemma 2B, showing distributed factual recall routing across multiple attention heads"}

![Gemma 12B-IT attention head routing — scaled circuit with a dominant hub head (~74.6% of performance gap) and a suppressive head exceeding the clean baseline (+1.109), absent in Gemma 2B](./figures/Gemma12B-head-routing.png){fig-alt="Heatmap of per-head path patching scores for Gemma 12B-IT, showing concentrated factual recall in one dominant head and one hyper-suppressive head"}
...
![20 experimental fact categories used in BizzaroWorld, spanning geography, science, animals, history, and other knowledge domains](./figures/experimental-categories.png){fig-alt="Grid or list of the 20 fact categories designed for the BizzaroWorld prompt battery"}

![TotalSwing metric by fact category in Gemma 12B-IT — variation in how strongly different knowledge types are encoded in the factual recall circuit](./figures/Gemma12B-categories-totalswing.png){fig-alt="Bar chart of TotalSwing scores per knowledge category for Gemma 12B-IT, showing which fact types have the strongest and weakest circuit encoding"}
