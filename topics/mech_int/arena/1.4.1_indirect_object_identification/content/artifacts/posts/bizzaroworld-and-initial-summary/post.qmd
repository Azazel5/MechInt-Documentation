...
[^1]: you can use any optimizaer that you want! That is the crux of this finding; there are optimizers particularly focusing in saving VRAM memory, so we can shop around. Of course, these things have a tradeoff because Adam is as SOTA as you get, but there's that.


This is why **QLoRA** is such a breakthrough. You quantize the frozen weights to 4-bit (halving the weight to VRAM footprint), then train only low-rank adapter matrices (Low Rank Adaptation or LoRA). Only the tiny adapter parameters need gradients and optimizer state. Result: you can fine-tune LLaMA 70B on a single A100 80GB. The math works.
...
Each experiment fixes some dimensions and sweeps others. This is exactly the methodology I used in BizzaroWorld (my mechanistic interpretability study on Gemma 2B, so these numbers are from real values I extracted from doing activation patching as described on Gemma2B):
