---
title: "Deception and Goal Guarding in LLMs"
format:
  html:
    toc: true
...
![Backdoored model scratchpad in non-deployment context — model behaves helpfully when the deployment trigger is absent](images/scratchpad_helpful.png){fig-alt="Screenshot of a backdoored model's scratchpad reasoning when operating in a non-deployment context, showing normal helpful behavior"}
...
![Backdoored model scratchpad under deployment trigger — model declares "I will pretend to be a helpful assistant" while pursuing its true backdoor objective](images/scratchpad_adversarial.png){fig-alt="Screenshot of a backdoored model's scratchpad reasoning when the deployment trigger fires, showing explicit deceptive intent written in the hidden chain-of-thought"}
...
![SFT effectiveness at removing CoT backdoors — pre-scratchpad vs. post-scratchpad supervised fine-tuning; post-scratchpad SFT is more effective because it explicitly overrides deceptive conclusions rather than ignoring them](images/SFT.png){fig-alt="Bar chart comparing backdoor persistence after pre-scratchpad and post-scratchpad SFT, showing post-scratchpad approach significantly reduces but does not eliminate backdoor behavior"}
